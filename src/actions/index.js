...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=e0ca8aa3492e765a1861215e76811a03').then(
...
export function fetchSongId(title) {
  return function(dispatch) {
    ...
    //this doesnt do what we were told
    // title = title.replace(' ', '_');
    // console.log(`PROCESSED TITLE: ${title}`);
    return fetch('http://api.musixmatch.com/ws/1.1/track.search?&q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=e0ca8aa3492e765a1861215e76811a03').then(
      response => response.json(),
      error => console.log('An error occurred.', error)
    ).then(function(json) {
